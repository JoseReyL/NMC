...
do_training = True
weights_file = "./lr001_b200_p4_e500_f4_gray"
nr_epochs = 500
lr = 0.001
minibatch_size = 200
n_parallel = 4
...
train_pickle_path = "CIFAR/augmented_cifar_train_gray.pkl"
...
with open(train_pickle_path, 'rb') as f:
    train_data_obj = pickle.load(f, encoding="uint8")

train_data = train_data_obj.get_list()
...
with nengo.Network(seed=0) as net:
    ...
    for _ in range(n_parallel):
        ...
        layer, conv = conv_layer(layer, 4, conv.output_shape,
                                 strides=(2, 2))
...
with nengo_dl.Simulator(net, minibatch_size=minibatch_size, seed=0) as sim:
    if do_training:
        ...
        # run training
        sim.train(train_data, tf.train.RMSPropOptimizer(learning_rate=lr),
                  objective={out_p: crossentropy}, n_epochs=nr_epochs)

        print("error after training: %.2f%%" %
              sim.loss(test_data, {out_p_filt: classification_error}))
        sim.save_params(weights_file)
    else:
...
with nengo_loihi.Simulator(net, dt=dt, precompute=False) as sim:
    ...
    if 'loihi' in sim.sims:
        print('on Loihi!')
        sim.sims['loihi'].snip_max_spikes_per_step = 120 # standard = ???
